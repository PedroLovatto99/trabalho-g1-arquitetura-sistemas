...
// ex: const paymentService = new PaymentService(new PaymentRepository())
const paymentService: IPaymentService = {} as any
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const params: {
      ...
      pageSize: Number.isFinite(pageSize) && pageSize > 0 ? Math.min(pageSize, 200) : 20,
    }
...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (!found) return res.status(404).json({ message: "Pagamento não encontrado" })
