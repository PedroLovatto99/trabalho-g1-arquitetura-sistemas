// src/services/ProductServiceHttpClient.ts
import { productsApi } from "./api"; // ✅ Importando sua instância configurada do Axios
...
export class StockValidationError extends Error {
  public unavailableProducts: Array<{
    productId: string;
    requested: number;
    available: number;
  }>;
...
export class ProductServiceHttpClient implements IProductApi {
  ...
  async adjustStock(productId: string, quantity: number): Promise<void> {
    try {
      ...
      throw new Error(
        `Não foi possível atualizar o estoque do produto ${productId}.`
      );
    }
  }
}